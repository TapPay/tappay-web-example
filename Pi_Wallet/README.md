# Pi Wallet with TapPay

## Required

1. 請搭配 Web SDk v5.10.0 使用 Pi Wallet, 舊版本不支援 Pi Wallet
2. 請到 TapPay Portal 申請帳號，取得 APP_ID 和 APP_KEY

## Overview

1. 前端用 `TPDirect.piWallet.getPrime()` 拿到 Pi Wallet 專屬的 `prime`
2. 前端把 `prime` 送到後端伺服器
3. 前端等待後端伺服器回傳 `payment_url` 並使用 `TPDirect.redirect(payment_url)` 的方式讓使用者進行 Pi Wallet 付款
...
使用 `TPDirect.piWallet.getPrime(callback)` 去拿 `prime`

```js
TPDirect.piWallet.getPrime(function(result) {
    // code
})
```
...
msg | String | 錯誤訊息
prime | String | prime 字串 `pi_` 開頭，於 <a href="https://docs.tappaysdk.com/pi-wallet/zh/back.html#pay-by-prime-api">Pay by Prime</a> 交易時使用
