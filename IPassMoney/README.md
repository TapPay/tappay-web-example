# iPass Money with TapPay

## Required

1. 請使用 Web SDK v5.21.0 以上版本使用 iPass Money, 舊版本不支援 iPass Money
2. 請到 TapPay Portal 申請帳號，取得 APP_ID 和 APP_KEY

## Overview

1. 前端用 `TPDirect.iPassMoney.getPrime()` 拿到 iPass Money 專屬的 `prime`
2. 前端等待後端伺服器回傳 `payment_url` 並使用 `TPDirect.redirect(payment_url)` 的方式讓使用者進行 iPass Money 付款
...
```html
<script src="https://js.tappaysdk.com/sdk/tpdirect/v5.21.0"></script>
```
...
使用 `TPDirect.iPassMoney.getPrime(callback)` 去拿 `prime`
...
<head>
    ...
    <title>iPass Money Example</title>
...
<div class="payment-view ui grid centered stackable doubling">
    ...
    <div class="ten wide column ">
        <div class="ui center aligned segment">
            <h2> iPass Money Test</h2>
            <button id='sendButton' class="ui button">Pay with iPass Money</button>
        </div>
...
-d '{
    ...
    "prime": "${result.prime}",
    "amount": "1",
    "merchant_id": "GlobalTesting_IPASS_MONEY_EC",
    "details": "Some item",
    "cardholder": {
